unit UZoo_ImportaPesagemWizard;
...
type
  TZoo_ImportaPesagemWizard = class(T_FormWizard)
    TabSheet1: TTabSheet;
    Label2: TLabel;
...
{ TfrmCustomWizard }
(*
procedure TZoo_ImportaPesagemWizard.Button1Click(Sender: TObject);
var
  source : string ;
begin
...
procedure TZoo_ImportaPesagemWizard.Button1Click(Sender: TObject);
begin
  JvSaveDialog1.FileName := 'Modelo Pesagens.xlsx' ;
  if JvSaveDialog1.Execute then
    begin
     ...
     SaveRes ( 'XLS_PESAGENS', JvSaveDialog1.FileName )  ;
     MessageDlg ( 'Modelo salvo em :' + JvSaveDialog1.FileName ) ;
    end;
end;


class function TZoo_ImportaPesagemWizard.Execute ( var AFileName : string ) : boolean;
var
  ThisForm: TZoo_ImportaPesagemWizard;
begin
  ThisForm := TZoo_ImportaPesagemWizard.Create ( nil ) ;
  try
...
procedure TZoo_ImportaPesagemWizard._actExecuteUpdate(Sender: TObject);
begin
...
procedure TZoo_ImportaPesagemWizard._actNextExecute(Sender: TObject);
begin
...
procedure TZoo_ImportaPesagemWizard._actPriorExecute(Sender: TObject);
begin
  // insira as validacoes antes do inherited
  //
