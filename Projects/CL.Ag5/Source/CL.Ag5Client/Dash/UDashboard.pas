...
uses
  ...
  CLAg5.AccessKeyGen,
  winsock,
  DataSnap.DBClient, Tc.DataSnap.DBClient.Helpers, Data.DB, Tc.Data.DB.Helpers ;

type

  //TCLAg5NSMsgModuloZootecnico = class ( TCLAg5NSMsg )
  ...
  private
    ...
    procedure ChangeDesktop( AMsg : TCLAg5NSMsg );
    procedure SetOptions ( AEnabled : boolean ) ;
    procedure CheckLicense ;
    procedure LoadWebCredentials;
    procedure PrepareIcons ;
    procedure UserInfoChanged(AMsg: TCLAg5NSMsg);
  public
    { Public declarations }
...
uses  Tc.VCL.Application, Tc.DBRTL.AbstractDB, ModulesDashboard ;

var
  GHasInternet : boolean = False ;

procedure TDashboard.LoadWebCredentials;
const
   _SELECT =
         'SELECT'
  +#13#10'MODULE,'
...
procedure TDashboard.Blur ( ABlur : boolean ) ;
var
  I : integer ;
begin
  if Assigned ( ModulesDashboard.FModuleSelectorImageStream ) then

  for I := Low( FOptionsList ) to High( FOptionsList ) do
    FOptionsList[ I ].BlurEffect.Enabled := ABlur ;

  SetOptions ( not ABlur ) ;

  if not ABlur then
     CheckLicense ;
end;

procedure TDashboard.ChangeBlur(AMsg: TCLAg5NSMsg);
begin
  Blur ( AMsg.ClassType = TCLAg5NSMsgDashBlurOn ) ;
end;

procedure TDashboard.ChangeDesktop(AMsg: TCLAg5NSMsg);
begin
  BackgroundImage.Bitmap.LoadFromStream ( TMemoryStream( AMsg.Sender ) ) ;
end;

procedure TDashboard.UserInfoChanged(AMsg: TCLAg5NSMsg);
begin
   CheckLicense ;
end;

procedure TDashboard.CheckLicense;
var
 A: Integer;
 LZootecnico : boolean ;
 LFinanceiro : boolean ;
...
procedure TDashboard.PrepareIcons ;
type
...
*)
  ...
  for I := Low( LLayouts ) to High( LLayouts ) do
    begin
      ...
      FOptionsList[ I ].Image.OnClick      := nil ; //ImageClick ;
      ...
      FOptionsList[ I ].InfoLabel.Font.Size    := 12 ;
      FOptionsList[ I ].InfoLabel.Position.X   := FOptionsList[ I ].Image.Position.X - 10 ;
      FOptionsList[ I ].InfoLabel.Position.Y   := FOptionsList[ I ].Image.Position.Y ;
      FOptionsList[ I ].InfoLabel.Size.Width   := FOptionsList[ I ].Image.Width + 10 ;
      ...
      FOptionsList[ I ].InfoLabel.TextSettings.FontColor   := TAlphaColorRec.Lightgray  ; //TAlphaColorRec.Gold ;
...
begin
  ...
  TCLAg5NSMsgBackgroundChanged.Subscribe( ChangeDesktop );
  TCLAg5NSMsgUserInfoChanged.Subscribe ( UserInfoChanged ) ;
  TCLAg5NSMsgDashBlurOn.Subscribe( ChangeBlur );
  TCLAg5NSMsgDashBlurOff.Subscribe( ChangeBlur );
...
procedure TDashboard.FormPaint(Sender: TObject; Canvas: TCanvas;
  const ARect: TRectF);
begin
...
procedure TDashboard.SetOptions ( AEnabled : boolean ) ;
var
 I : integer ;
begin
  if AEnabled then
    for I := Low( FOptionsList ) to High( FOptionsList ) do
      begin
        FOptionsList[ I ].Image.Cursor       := crHandPoint ;
    ...
    for I := Low( FOptionsList ) to High( FOptionsList ) do
      begin
        FOptionsList[ I ].Image.Cursor       := crDefault ;
        FOptionsList[ I ].Image.OnClick      := nil ;
        FOptionsList[ I ].Image.OnMouseEnter := nil ;
        FOptionsList[ I ].Image.OnMouseLeave := nil ;
      end
end ;
...
procedure TDashboard.ImageClick(Sender: TObject);
var
  LIndex : integer ;
begin
...
procedure TDashboard.ImageMouseLeave(Sender: TObject);
var
  LIndex : integer ;
begin
   LIndex := TImage ( Sender ).Tag ;
...
procedure TDashboard.ImageMouseEnter(Sender: TObject);
var
  LIndex : integer ;
begin
   LIndex := TImage ( Sender ).Tag ;
