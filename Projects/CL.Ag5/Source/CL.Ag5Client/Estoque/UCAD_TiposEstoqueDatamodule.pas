unit UCAD_TiposEstoqueDatamodule;
...
type
  TCAD_TiposEstoqueDatamodule = class(T_dtmBASEDBListEdit)
...
procedure TCAD_TiposEstoqueDatamodule.OpenTables;
begin
  inherited;
//  SetParams;
end;

procedure TCAD_TiposEstoqueDatamodule.SetFilter(TipoCadastro: Integer);
begin
...
procedure TCAD_TiposEstoqueDatamodule.SetParams;
begin
...
function TCAD_TiposEstoqueDatamodule.ValidaDescricao(FDescricao: string): Boolean;
const
  SQL_CADASTROS = 'SELECT COUNT(1) RESULT ' +
                  'FROM CAD_TIPOS ' +
                  'WHERE KSYS$DOMAIN = :KSYS$DOMAIN ' +
                  '  AND TIPO = :TIPO ' +
                  '  AND UPPER(DESCRICAO) = :DESCRICAO ';
var
  cds : TClientDataSet;
    p : TParams;
begin
  ...
  p.CreateParam( ftString,  'DESCRICAO',    ptInput ).AsString  := UpperCase(FDescricao);

  TTcAbstractDB.GetByAlias('FINANCEIRO').populateClientDataSet( cds, SQL_CADASTROS, p );
...
procedure TCAD_TiposEstoqueDatamodule.cdsLstTiposCadastrosNewRecord(DataSet: TDataSet);
begin
  inherited;
  self.InitializeKeys ;
end;

procedure TCAD_TiposEstoqueDatamodule.cdsTiposCadastrosBeforePost(DataSet: TDataSet);
begin
...
procedure TCAD_TiposEstoqueDatamodule.cdsTiposCadastrosNewRecord(DataSet: TDataSet);
begin
  inherited;
